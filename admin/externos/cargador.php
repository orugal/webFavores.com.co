		...
		//var_dump($array_final2);
			...
			foreach($array_final2 as $key=>$productos)
			{
				...
				//verifico si el codigo de producto que lleva el documento ya existe
					...
					$valores1		 =	str_replace('"','',$valores1);
					$valores1		 =	str_replace('""','',$valores1);
				$existe		=	$funciones->consultaUniversal('mails',' mail="'.$valores1.'"');
				...
				if(count($existe) > 0)
				{
					...
					$query_update_producto	=	$db->Execute(sprintf("UPDATE mails SET %s WHERE id=%s",$set,$existe[0]['id']));
				...
				else
				{
				   ...
				   //elimino la ultima coma de cada variable que concatene
					...
					//echo $campos;
					$query_guardado_producto	=	sprintf("INSERT INTO mails (%s) VALUES(%s)",$campos,$valores);
