...
$filtro		=	'';
	 ...
	 elseif(isset($_POST['exportar']))//exportar los usuarios
	 {
		...
		//die($filtro);
		$query_exportar	=	$db->GetAll(sprintf("SELECT * FROM fiat WHERE  %s",$filtro));
	 ...
	 else
	 {
		...
		if(isset($_POST['accion']))
		{
			...
			$query_borrado	=	$db->Execute(sprintf("DELETE FROM fiat WHERE id in(%s)",implode(',',$_POST['elim']))) or die(sprintf("DELETE FROM fiat WHERE id in(%s)",implode(',',$_POST['elim'])));
			if($query_borrado)
			{
	 ...
	 if(!empty($filtro))
	 {
	 	 $query_usuarios	=	$db->Execute(sprintf("SELECT * FROM fiat WHERE  %s ORDER BY id DESC",$filtro));
	 	// echo sprintf("SELECT * FROM concurso WHERE  %s",$filtro);
	 }
	 else
	 {
		 $query_usuarios	=	$db->Execute(sprintf("SELECT * FROM fiat ORDER BY id DESC")); 	
	 }
