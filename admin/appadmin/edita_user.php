...
elseif(isset($_GET['new']))
{
	if(isset($_POST['guardar']))
	{
		...
		if(!empty($email) and !ereg('([0-9a-zA-Z]+([_.-]?[0-9a-zA-Z]+)*@[0-9a-zA-Z]+[0-9,a-z,A-Z,.,-]*(.){1}[a-zA-Z]{2,4})+',$email))
		{
			echo "<script>alert('La sintanxis del correo principal es incorrecta');</script>";
		}
		elseif(!empty($telefono) and !is_numeric($telefono))
		{
			echo "<script>alert('Debe Seleccionar un minisite para este usuario');</script>";
		}
		else
		{
			...
			if($resultado)
			{
				echo "<script>alert('".strip_tags($resultado)."');window.parent.closeShadow();location.reload();</script>";
			}
		}
	}
}
			...
			<table width="100%" cellspacing="5" cellpadding="0">
				<tr>
					<td style="text-align:justify" colspan="2">
						Por favor complete el formulario para realizar la edición de el usuario seleccionado.<br>
				...
				<tr>
					...
					<td class="cajas">
						<input type="text" readonly="readonly" name="username" <?if(!isset($_GET['new'])){?>readonly<?} ?> value="<?=$username ?>">
					</td>
				</tr>
				<tr>
					<td class="labels">
