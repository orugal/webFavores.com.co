...
$filtro		=	'';
	 ...
	 elseif(isset($_POST['exportar']))//exportar los usuarios
	 {
		...
		//die($filtro);
		$query_exportar	=	$db->GetAll(sprintf("SELECT * FROM concurso WHERE  %s",$filtro));
	 ...
	 else
	 {
		...
		if(isset($_POST['accion']))
		{
			...
			$query_borrado	=	$db->Execute(sprintf("UPDATE concurso SET eliminado=1 WHERE id_concurso in(%s)",implode(',',$_POST['elim']))) or die(sprintf("DELETE FROM concurso WHERE idusuario in(%s)",implode($_POST['elim'])));
			if($query_borrado)
			{
	 ...
	 if(!empty($filtro))
	 {
	 	 $query_usuarios	=	$db->Execute(sprintf("SELECT * FROM concurso WHERE  %s AND eliminado=0 ORDER BY id_concurso DESC",$filtro));
	 	// echo sprintf("SELECT * FROM concurso WHERE  %s",$filtro);
	 }
	 else
	 {
		 $query_usuarios	=	$db->Execute(sprintf("SELECT * FROM concurso WHERE eliminado=0 ORDER BY id_concurso DESC")); 	
	 }
		...
		foreach($array_users as $datos)
		{
			if($datos['idusuario'] != 1)
			{
				...
				echo "<td align='center'>
							 <a href='../facebook4/fotos/".$datos['foto']."' rel='Shadowbox;'>Ver Foto</a>
						</td>";
				if($datos['activo'] == 1){
					echo "<td align='center' id='ojo".$datos['id_concurso']."'>
							<a style='cursor:pointer' onClick='habilita(".$datos['id_concurso'].",".$datos['activo'].")'><img src='images/eye.png' border='0'></a>
				      	</td>";
				}else{
					echo "<td align='center' id='ojo".$datos['id_concurso']."'>
							<a style='cursor:pointer' onClick='habilita(".$datos['id_concurso'].",".$datos['activo'].")'><img src='images/eye_cerrado.png' border='0'></a>
				      	</td>";
				}
				echo "<td align='center'>
						<input type='checkbox' name='elim[]' value='".$datos['id_concurso']."'>
				      </td>";
				echo "<tr>";
			}
		}	
